"""Validation-only screen for scalar versus component-wise graph gates.

This script keeps the frozen NLinear backbone, graph construction, EWMA span, and
graph residual adapter identical to M29, except for the gate shape.  It is meant
for low-cost model selection on the validation split before any frozen-test run.
"""
...
class ComponentGateGraphAdapter(nn.Module):
    def __init__(self, base: nn.Module, nodes: int, hidden: int, horizon: int, gate_mode: str):
        super().__init__()
        if gate_mode not in {"scalar", "component"}:
            raise ValueError(f"unsupported gate_mode={gate_mode}")
        self.base, self.nodes, self.horizon, self.gate_mode = base, nodes, horizon, gate_mode
        ...
        gate_shape = (3,) if gate_mode == "component" else ()
    ...
    def forward(self, values: torch.Tensor, high: torch.Tensor, mask: torch.Tensor, graph: torch.Tensor):
        ...
        if self.gate_mode == "component":
            correction = correction * gate.view(1, 1, 1, 3)
        else:
            correction = correction * gate
        return base_prediction + correction, gate
...
def metric_block(prediction: np.ndarray, truth: np.ndarray, valid: np.ndarray) -> dict[str, dict[str, float]]:
    output: dict[str, dict[str, float]] = {}
    valid4 = valid[..., None].repeat(3, axis=-1)
    for index, name in enumerate(COMPONENTS):
        use = valid4[..., index] & np.isfinite(truth[..., index])
        error = prediction[..., index][use] - truth[..., index][use]
        output[name] = {
    ...
    use_all = valid4 & np.isfinite(truth)
    error_all = prediction[use_all] - truth[use_all]
    output["overall"] = {
        "rmse_mm": float(np.sqrt(np.mean(error_all**2))),
        "mae_mm": float(np.mean(np.abs(error_all))),
        "p95_abs_error_mm": float(np.percentile(np.abs(error_all), 95)),
        "samples": int(use_all.sum()),
    }
    return output


def evaluate_validation(model: ComponentGateGraphAdapter, base: nn.Module, dataset: AdapterWindows, graph: torch.Tensor, raw: np.ndarray, scale: np.ndarray, center: np.ndarray, device: torch.device) -> tuple[dict[str, dict[str, float]], dict[str, dict[str, float]], object]:
    model.eval()
    base.eval()
    ...
    if last_gate is None:
        gate_value = None
    elif last_gate.ndim == 0:
        gate_value = float(last_gate.detach().cpu())
    else:
        gate_value = {name: float(value) for name, value in zip(COMPONENTS, last_gate.detach().cpu().numpy().tolist())}
    return metric_block(prediction_mm, truth, valid), metric_block(base_mm, truth, valid), gate_value
...
def main() -> None:
    ...
    parser.add_argument("--gate-mode", choices=("scalar", "component"), required=True)
    ...
    model = ComponentGateGraphAdapter(base, raw.shape[1], args.hidden, horizon, args.gate_mode).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        gate = torch.sigmoid(model.graph_gate_logit).detach().cpu()
        gate_payload: object = float(gate) if gate.ndim == 0 else {name: float(value) for name, value in zip(COMPONENTS, gate.numpy().tolist())}
        record = {
            "epoch": epoch,
            "train_loss": train_loss,
            "validation_loss": validation_loss,
            "graph_gate": gate_payload,
    ...
    torch.save({"state_dict": model.state_dict(), "config": config, "best_validation_loss": best_loss}, out / "best.pt")
    payload = {
        "device": str(device),
