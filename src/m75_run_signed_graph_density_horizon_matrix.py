"""Run the signed-graph MS-GRAN horizon-density matrix.

This script evaluates the signed-correlation graph variant after the primary
1-day/90-station screen. It retrains only the residual graph adapter for each
horizon/network-density/seed cell, using matched frozen NLinear checkpoints.
"""
...
def run_name(horizon: int, stations: int, seed_index: int) -> str:
    return f"M75_msgran_signed_{horizon}day_{stations}stations_seed{seed_index}"


def invoke(command: list[str], log) -> None:
    message = "[M75] " + " ".join(command)
...
def summarize(rows: list[dict[str, object]], horizons: list[int], sizes: list[int]) -> list[dict[str, object]]:
    ...
    for horizon in horizons:
        for stations in sizes:
            ...
            for key in ("nlinear_rmse_mm", "msgran_signed_rmse_mm", "gain_vs_nlinear_percent", "skill_vs_persistence_percent"):
...
def main() -> None:
    ...
    log_path = REPORTS / "M75_signed_graph_density_horizon_matrix.log"
        ...
        for horizon in horizons:
            for stations in sizes:
                for seed_index in seed_indices:
                    ...
                    rows.append(
                        {
                            ...
                            "seed": seed,
                            "base_run": base_name,
                            "msgran_signed_run": name,
                            "nlinear_rmse_mm": base_rmse,
                            "msgran_signed_rmse_mm": model_rmse,
                    ...
                    partial = REPORTS / "M75_signed_graph_density_horizon_matrix_partial.json"
                    partial.write_text(
                        json.dumps({"runs": rows, "summary": summarize(rows, horizons, sizes)}, ensure_ascii=False, indent=2),
                        encoding="utf-8",
                    )

    destination = REPORTS / "M75_signed_graph_density_horizon_matrix.json"
